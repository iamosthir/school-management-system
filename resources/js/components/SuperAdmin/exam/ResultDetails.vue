...
export default {
  ...
  methods: {
    getExamResultInfo() {
      axios.get(`/admin/api/get-result?exam=${this.$route.params.examId}&student=${this.$route.params.studentId}`).then(resp=>{
        return resp.data;
      }).then(data=>{
        if(data.status == "ok") {
  ...
  mounted() {
    ...
    $('#datatable').DataTable({
      processing: true,
      serverside:true,
      ajax: '/admin/api/get-exam-result-datatable?examId='+_self.$route.params.examId+'&student='+_self.$route.params.studentId,
      lengthChange : true,
      columns: [
        { data: "question" },
        { data: "answer" },
